...
function toAsciiOnly(s) {
	const original = s;
	let out = s;
	for (const [nonAscii, ascii] of Object.entries(require_fuzzyDomainMatching.TRANSLITERATION_MAP)) out = out.replace(new RegExp(nonAscii, "g"), ascii);
	out = out.replace(/[^ -~]/g, "");
	return {
		out,
...
function changeCodeToReason(code) {
	switch (code) {
		case require_fuzzyDomainMatching.EmailChangeCodes.NORMALISED_UNICODE_SYMBOLS: return "Replaced unicode symbols.";
		case require_fuzzyDomainMatching.EmailChangeCodes.INVALID_EMAIL_SHAPE: return "Invalid email format.";
		case require_fuzzyDomainMatching.EmailChangeCodes.STRIPPED_DISPLAY_NAME_AND_COMMENTS: return "Removed display name or comments.";
		case require_fuzzyDomainMatching.EmailChangeCodes.DEOBFUSCATED_AT_AND_DOT: return "Fixed obfuscated \"at\" or \"dot\" substitutions.";
		case require_fuzzyDomainMatching.EmailChangeCodes.TIDIED_PUNCTUATION_AND_SPACING: return "Tidied punctuation and spacing.";
		case require_fuzzyDomainMatching.EmailChangeCodes.FIXED_DOMAIN_AND_TLD_TYPOS: return "Corrected common domain or TLD typos.";
		case require_fuzzyDomainMatching.EmailChangeCodes.FUZZY_DOMAIN_CORRECTION: return "Corrected domain using fuzzy matching.";
		case require_fuzzyDomainMatching.EmailChangeCodes.LOWERCASED_DOMAIN: return "Lowercased domain part.";
		case require_fuzzyDomainMatching.EmailChangeCodes.BLOCKED_BY_LIST: return "Email is blocked.";
		case require_fuzzyDomainMatching.EmailChangeCodes.CONVERTED_TO_ASCII: return "Converted non-ASCII characters to ASCII.";
		default:
...
function performFuzzyDomainNormalization(email, config) {
	if (!config.enabled || !require_validateEmail.looksLikeEmail(email)) return {
...
function normaliseEmail(raw, options = {}) {
	...
	if (require_validateEmail.isEmpty(s)) return {
		email: s,
		valid: false,
		changes,
		changeCodes: []
	};
	{
		...
		if (r.changed) {
			s = r.out;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.NORMALISED_UNICODE_SYMBOLS);
		}
	}
	{
		...
		if (r.changed) {
			s = r.out;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.STRIPPED_DISPLAY_NAME_AND_COMMENTS);
		}
	}
	{
		...
		if (r.changed) {
			s = r.out;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.DEOBFUSCATED_AT_AND_DOT);
		}
	}
	{
		...
		if (r.changed) {
			s = r.out;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.TIDIED_PUNCTUATION_AND_SPACING);
		}
	}
	{
		const r = applyMaps(s, {
		...
		if (r.changed) {
			s = r.out;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.FIXED_DOMAIN_AND_TLD_TYPOS);
		}
	}
	...
	if (fuzzyConfig) {
		...
		if (fuzzyResult.wasChanged) {
			...
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.FUZZY_DOMAIN_CORRECTION);
		}
	}
	if (asciiOnly) {
		...
		if (r.changed) {
			s = r.out;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.CONVERTED_TO_ASCII);
		}
	}
	const at = s.indexOf("@");
	if (at > -1) {
		...
		if (next !== s) {
			s = next;
			changes.push(require_fuzzyDomainMatching.EmailChangeCodes.LOWERCASED_DOMAIN);
		}
	}
	const cfg = options.blocklist || require_fuzzyDomainMatching.DEFAULT_BLOCKLIST;
	if (require_validateEmail.blocklisted(s, cfg)) return {
		email: s,
		valid: false,
		changeCodes: [...changes, require_fuzzyDomainMatching.EmailChangeCodes.BLOCKED_BY_LIST],
		changes: mapChangeCodesToReason([...changes, require_fuzzyDomainMatching.EmailChangeCodes.BLOCKED_BY_LIST])
	};
	if (!require_validateEmail.looksLikeEmail(s)) {
		changes.push(require_fuzzyDomainMatching.EmailChangeCodes.INVALID_EMAIL_SHAPE);
		return {
			email: s,
			valid: false,
			changeCodes: [...changes, require_fuzzyDomainMatching.EmailChangeCodes.INVALID_EMAIL_SHAPE],
			changes: mapChangeCodesToReason([...changes, require_fuzzyDomainMatching.EmailChangeCodes.INVALID_EMAIL_SHAPE])
		};
	}
	return {
		email: s,
...
//#endregion
Object.defineProperty(exports, 'changeCodeToReason', {
  enumerable: true,
  get: function () {
    return changeCodeToReason;
  }
});
Object.defineProperty(exports, 'normaliseEmail', {
  enumerable: true,
  get: function () {
    return normaliseEmail;
  }
});
//# sourceMappingURL=normaliseEmail-gYqVFJ9w.cjs.map