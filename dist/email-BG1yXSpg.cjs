...
function resolve(binding, el) {
	...
	return {
		opts: {
			...
			blocklist: {
				...require_fuzzyDomainMatching.DEFAULT_BLOCKLIST,
				...value.blocklist || {}
			},
			fixDomains: {
...
var email_default = {
	mounted(el, binding) {
		...
		const run = (raw) => {
			const r = require_normaliseEmail.normaliseEmail(raw, opts);
	...
	updated(el, binding) {
		...
		const r = require_normaliseEmail.normaliseEmail(input.value || "", opts);
...
//#endregion
Object.defineProperty(exports, 'email_default', {
  enumerable: true,
  get: function () {
    return email_default;
  }
});
//# sourceMappingURL=email-BG1yXSpg.cjs.map