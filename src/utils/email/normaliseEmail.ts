...
function toAsciiOnly(s: string): EmailFixResult {
  ...
  for (const [nonAscii, ascii] of Object.entries(TRANSLITERATION_MAP)) {
...
export function normaliseEmail(
  ...
  {
    const r = applyMaps(s, {
      domains: { ...DEFAULT_FIX_DOMAINS, ...(options.fixDomains || {}) },
      tlds: { ...DEFAULT_FIX_TLDS, ...(options.fixTlds || {}) },
    })
    if (r.changed) {
      s = r.out
