...
For third-party libraries with complex interfaces, we use simplified mocking strategies:
...
Uses **tsdown** for fast dual-builds and types.
...
- Extend `DEFAULT_FUZZY_DOMAIN_CANDIDATES` in `constants.ts` if adding built-in domains
